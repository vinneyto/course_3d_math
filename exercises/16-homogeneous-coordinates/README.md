...
В компьютерной графике запись `(1, 2, 0, 0)` используется для представления
вектора с компонентами `(1, 2, 0)`.

## Почему нельзя разделить на ноль

При `w = 0` деление `x / w`, `y / w`, `z / w` не определено. Мы не делим на
ноль, а рассматриваем, что происходит, когда ненулевое `w` становится всё
меньше и приближается к нулю.

Значение с `w = 0` нельзя преобразовать в обычную конечную точку. Оно
представляет направление. Для такого вектора первые три компоненты используются
непосредственно:

```text
homogeneousVector = (3, 4, 0, 0)
vector = (3, 4, 0)
```
